...
pub fn load(allocator: std.mem.Allocator, filename: []const u8, n_ctx: u32) !Model {
    ...
    model.hparams.n_vocab = try file.reader().readIntLittle(u32);
    model.hparams.n_embd = try file.reader().readIntLittle(u32);
    model.hparams.n_head = try file.reader().readIntLittle(u32);
    model.hparams.n_layer = try file.reader().readIntLittle(u32);
    model.hparams.n_rot = try file.reader().readIntLittle(u32);
    model.hparams.use_parallel_residual = try file.reader().readIntLittle(u32);
    model.hparams.f16 = try file.reader().readIntLittle(u32);
    ...
    for (0..model.hparams.n_vocab) |_| {
        const len = try file.reader().readIntLittle(u32);
    ...
    const wtype: c.enum_ggml_type = switch (model.hparams.f16) {
        ...
        3 => c.GGML_TYPE_Q4_1,
            ...
            std.log.warn("{s}: invalid model file \"{}\" (bad f16 value {d})", .{ @src().fn_name, std.zig.fmtEscapes(filename), w });
    ...
    {
        ctx_size += model.hparams.n_embd * c.ggml_type_size(c.c.GGML_TYPE_F32); // ln_f_g
        ctx_size += model.hparams.n_embd * c.ggml_type_size(c.c.GGML_TYPE_F32); // ln_f_b

        ctx_size += model.hparams.n_embd * model.hparams.n_vocab * c.ggml_type_size(wtype); // wte

        ctx_size += model.hparams.n_embd * model.hparams.n_vocab * c.ggml_type_size(wtype); // lmh_g
        ...
        ctx_size += model.hparams.n_ctx * model.hparams.n_layer * model.hparams.n_embd * c.ggml_type_size(c.GGML_TYPE_F32); // memory_k
        ctx_size += model.hparams.n_ctx * model.hparams.n_layer * model.hparams.n_embd * c.ggml_type_size(c.GGML_TYPE_F32); // memory_v
    ...
    const mem_buffer = try allocator.alloc(u8, ctx_size);
    errdefer allocator.free(mem_buffer);
    var params = c.ggml_init_params{
        .mem_size = ctx_size,
        .mem_buffer = mem_buffer.ptr,
        .no_alloc = false,
    };
    ...
    // load weights
    {
        ...
        while (true) {
            const n_dims = try file.reader().readIntLittle(i32);
            const name_length = try file.reader().readIntLittle(i32);
            const ftype = try file.reader().readIntLittle(i32);
            ...
            var shape = [2]u32{ 1, 1 };
            for (&shape) |*ne| {
                ne.* = try file.reader().readIntLittle(u32);
                nelements *= ne.*;
            }

            try name_buffer.resize(allocator, name_length);
            try file.reader().readNoEof(name_buffer.items);
            ...
            if (std.mem.eql(u8, layer_or_other, "layers")) {
                ...
                } else if (std.mem.eql(u8, layer_desc, "attention.dense_h_to_4h.weight")) {
                    tensor = &layer.c_mlp_fc_w;
                } else if (std.mem.eql(u8, layer_desc, "attention.dense_h_to_4h.bias")) {
                    tensor = &layer.c_mlp_fc_w;
                } else if (std.mem.eql(u8, layer_desc, "attention.dense_4h_to_h.weight")) {
                    tensor = &layer.c_mlp_proj_w_trans;
                } else if (std.mem.eql(u8, layer_desc, "attention.dense_4h_to_h.bias")) {
                    tensor = &layer.c_mlp_proj_b;
                }
            } else if (std.mem.eql(u8, layer_or_other, "hello, world")) {}
            ...
            const bpe_size = switch (ftype) {
                ...
                3 => c.ggml_type_size(c.GGML_TYPE_Q4_1),
                    ...
                    std.log.warn(" {s}: unknown ftype {} in model file", .{ @src().fn_name, f });
                    return error.UnknownFType;
                },
            };

            if ((nelements * bpe_size) / @intCast(usize, c.ggml_blck_size(tensor.?.*.type)) != c.ggml_nbytes(tensor.?.*)) {
                std.log.warn(" {s}: tensor \"{}\" has wrong size in model file, expected {d}, got {d}", .{ @src().fn_name, std.zig.fmtEscapes(name_buffer.items), c.ggml_nbytes(tensor.?.*), nelements * bpe_size });
                return error.InvalidModelSize;
            }

            try file.reader().readNoEof(@ptrCast([*]u8, tensor.?.*.data)[0..c.ggml_nbytes(tensor.?.*)]);
